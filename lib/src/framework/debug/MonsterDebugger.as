	...
	public class MonsterDebugger
	{
		...
		private function snapshotInternal(target:DisplayObject, color:uint = 0x111111):void
		{
			if (enabled)
			{
				...
				var bitmapData:BitmapData = new BitmapData(target.width||1, target.height||1);
				bitmapData.draw(target);
				
				// Write the bitmap in the bytearray
				var bytes:ByteArray = bitmapData.getPixels(new Rectangle(0, 0, target.width||1, target.height||1));
				
				//Create a send object
				send({text:COMMAND_SNAPSHOT, date:new Date(), target:String(target), bytes:bytes, width:target.width||1, height:target.height||1, color:color});
